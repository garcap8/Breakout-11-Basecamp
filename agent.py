...
def build_tools() -> List[Dict[str, Any]]:                 # ✏️ Build 1, step 1.3
    ...
    return [
        ...
        },
        {
            ...
            "description": (
                "Find rebookable flight options for the customer's disrupted segment on this "
                "PNR: other nonstop departures on the same origin-destination and cabin, on or "
                "after the affected date, with the current booking's own flight excluded. Fare "
                "family, loyalty tier, and party size are re-derived from the booking, not taken "
                "as arguments. Call this after lookup_booking, once you know the segment needs "
                "rebooking, and before offering the customer a specific option or holding a seat."
            ),
            "input_schema": {
                "type": "object",
